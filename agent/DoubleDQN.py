...
class DoubleDQAgent:
    ...
    def train(self, episode_buffer_training):
        ...
        with torch.no_grad():
            ...
            q_net = self.net(image_next, data_next)
            max, index = torch.max(q_net, dim=1)
            q_next = q_next[torch.arange(len(index)), index].unsqueeze(1)
    ...
    def work(self):
        ...
        for i in range(self.episode):
            self.epsilon = min(self.epsilon + 0.00004, 1)
            if i % 100 == 0:
                print(f'episode:{i}')
            self.episode_buffer = []
            self.env.reset()
