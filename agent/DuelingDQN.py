...
class DuelingDQAgent:
    ...
    def train(self, episode_buffer_training):
        ...
        data = torch.tensor(np.array([l[0][1] for l in episode_buffer_training])).float().to(self.device)
    ...
    def work(self):
        ...
        for i in range(self.episode):
            ...
            while self.env.done != True:
                ep_len += 1
                image_train = torch.from_numpy(s[0].transpose((2, 0, 1))).float().unsqueeze(0).to(
                    self.device)
